...
int main(void)
{
  ...
  ;
  rtmSetErrorStatus(part2c_starter_M, 0);
  ...
  part2c_starter_initialize();
  globalInterruptDisable();
  globalInterruptEnable();

  /* External Mode initialization */
  errorCode = extmodeInit(part2c_starter_M->extModeInfo,
    (extmodeSimulationTime_T *)rteiGetPtrTFinalTicks
    (part2c_starter_M->extModeInfo));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(part2c_starter_M, true);
    }
  }
  ...
  runModel =
    ...
    !rtmGetStopRequested(part2c_starter_M);
  enableTimer0Interrupt();
  config_ePWM_TBSync();
  globalInterruptEnable();
  while (runModel) {
