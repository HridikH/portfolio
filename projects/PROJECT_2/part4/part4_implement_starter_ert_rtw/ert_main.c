...
int main(void)
{
  ...
  ;
  rtmSetErrorStatus(part4_implement_starter_M, 0);
  ...
  part4_implement_starter_initialize();
  globalInterruptDisable();
  globalInterruptEnable();

  /* External Mode initialization */
  errorCode = extmodeInit(part4_implement_starter_M->extModeInfo,
    (extmodeSimulationTime_T *)rteiGetPtrTFinalTicks
    (part4_implement_starter_M->extModeInfo));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(part4_implement_starter_M, true);
    }
  }
  ...
  runModel =
    ...
    !rtmGetStopRequested(part4_implement_starter_M);
  enableTimer0Interrupt();
  config_ePWM_TBSync();
  globalInterruptEnable();
  while (runModel) {
  ...
  /* Terminate model */
  part4_implement_starter_terminate();
